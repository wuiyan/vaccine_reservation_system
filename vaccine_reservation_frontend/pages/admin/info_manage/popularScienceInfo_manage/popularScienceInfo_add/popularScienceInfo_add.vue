<template>
  <view class="flex-col page">
    <view class="flex-col self-stretch">
      <view class="flex-col justify-start items-center text-wrapper"><text class="text">新增科普信息</text></view>
...
<!--              <input type="file" accept="image/*" @change="onImageSelect"  placeholder="请选择封面图"/>-->
    ...
    <view class="flex-col justify-start text-area text-wrapper_2 mt-29" @tap="popularScienceInfoSubmit">
      <text class="font text_4">新增文章</text>
    </view>
  </view>
</template>
...
export default {
  ...
  methods: {
    ...
    popularScienceInfoSubmit() {
      ...
      const url = `popular`;
      request(url, "POST",this.inputForm)
          .then(responseData => {
            if (responseData.code == 0) {
              ...
              this.showToast("新增科普信息失败："+responseData.message)
            }
          })
          .catch(error => {
